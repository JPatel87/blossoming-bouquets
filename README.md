...
| Checkout page - (large screen sizes) ![Checkout order summary](readme_documents/features/checkout-order-summary.png) | 1) The checkout page has a form for the user to add their name, email, delivery and card details and card on the left hand side and the order summary is on the right hand side. 2) On medium and lower screens, the content vertically stacks; order summary first followed by the delivery and payment form. 3) The Country field is a django CountryField dropdown list. 4) A red alert message is activated to warn users of the amount the card will be charged. 4) The order summary includes the image of the bouquet, name, quantity and totals. 
